...
def houfline(img):
    ...
    l_brown = np.array([[25,100,100]])
    h_brown = np.array([[50,200,200]])
    mask_b = cv.inRange(hsv, l_brown, h_brown)
    #mask_g = cv.inRange(hsv, l_green, h_green)
    #res = cv.bitwise_or(mask_b, mask_g)
    #cv.imshow("test", res)
    cv.imshow("mask", mask_b)
    list1 = []
    edges = cv.Canny(mask_b,50,150,apertureSize = 3)
    cv.imshow("mask2", edges)
    #lines = cv.HoughLinesP(mask_b,1,np.pi/180,100,minLineLength=100,maxLineGap=1)
...
def houfline_black(img):
    ...
    cv.imshow("img1",img)
    #hsv = cv.cvtColor(img, cv.COLOR_BGR2HSV)
    l_black = np.array([[40,100,100]])
    h_black = np.array([[120,150,160]])
    ...
    cv.destroyAllWindows()
    return img
img = cv.imread("E:\\code\\data2\\0"+str(3)+".jpg")
list = houfline_black(img)